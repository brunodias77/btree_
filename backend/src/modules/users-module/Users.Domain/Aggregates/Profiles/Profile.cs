...
namespace Users.Domain.Aggregates.Profiles;
/// <summary>
...
public sealed class Profile : AggregateRoot<Guid>
{
    ...
    public static Profile Create(Guid userId, string email, string name, string? firstName = null, string? lastName = null)
    {
        ...
        profile.RegisterDomainEvent(new ProfileCreatedDomainEvent(profile.Id, userId, email, name));
        ...
        DateTime birthDate)
    {
        ...
        RegisterDomainEvent(new ProfileCreatedDomainEvent(Id, UserId));
    }
        ...
        bool? newsletterSubscribed)
    {
        ...
        RegisterDomainEvent(new ProfileUpdatedDomainEvent(Id, UserId));
    }
    ...
    public void RequestPasswordReset(string email, string resetToken, string resetCode)
    {
        PasswordResetToken = resetToken;
        PasswordResetCode = resetCode;
        PasswordResetCodeExpiration = DateTime.UtcNow.AddHours(2);
        
        IncrementVersion();

        RegisterDomainEvent(new ForgotPasswordRequestedDomainEvent(
            aggregateId: Id,
            userId: UserId,
            email: email,
            resetToken: resetToken,
            resetCode: resetCode,
            expirationDate: PasswordResetCodeExpiration.Value
        ));
    }

    public bool VerifyPasswordResetCode(string code)
    {
        if (PasswordResetCode != code) return false;
        if (PasswordResetCodeExpiration < DateTime.UtcNow) return false;
    ...
    public void ConfirmPasswordReset()
    {
        ...
        RegisterDomainEvent(new PasswordResetDomainEvent(
            aggregateId: Id,
            userId: UserId
        ));
    }

    public void RegisterPasswordChange()
    {
        RegisterDomainEvent(new PasswordChangedDomainEvent(
            aggregateId: Id,
            userId: UserId
        ));
    }
