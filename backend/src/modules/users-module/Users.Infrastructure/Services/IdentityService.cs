...
public class IdentityService : IIdentityService
{
    ...
    private readonly ITokenGenerator _tokenService; // Usado para gerar tokens de confirmação (se não for nativo do Identity)
    ...
    public async Task<Result<Guid>> CreateUserAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        ...
        // Gerar token de confirmação de email do Identity
        var identityToken = await _userManager.GenerateEmailConfirmationTokenAsync(user);

        // Gerar código curto (6 dígitos) para enviar ao usuário
        var shortCode = new Random().Next(100000, 999999).ToString();

        // Persistir o mapeamento (ShortCode -> IdentityToken) no banco
        var tokenEntity = new EmailConfirmationToken(
            user.Id, 
            identityToken, 
            shortCode, 
            DateTime.UtcNow.AddMinutes(15) // Expira em 15 minutos
        );

        _dbContext.EmailConfirmationTokens.Add(tokenEntity);
        await _dbContext.SaveChangesAsync(cancellationToken);

        // Enviar email APENAS com o código curto
        await _emailService.SendEmailAsync(
            email, 
            "Confirmação de cadastro - BCommerce", 
            $"Olá! Para confirmar seu cadastro, utilize o seguinte código de confirmação:\n\n{shortCode}\n\nEste código expira em 15 minutos."
        );
    ...
    public async Task<Result> ConfirmEmailAsync(string code, CancellationToken cancellationToken = default)
    {
        // Busca o token curto no banco para recuperar o token do Identity
        var storedToken = await _dbContext.EmailConfirmationTokens
            .Where(t => t.ShortCode == code && !t.IsUsed)
            .OrderByDescending(t => t.ExpiresAt) // Pega o mais recente se houver múltiplos
            .FirstOrDefaultAsync(cancellationToken);

        if (storedToken is null)
        {
             return Result.Failure(new Error("Identity.InvalidToken", "Código de confirmação inválido."));
        }

        if (!storedToken.IsValid())
        {
             return Result.Failure(new Error("Identity.TokenExpired", "Código de confirmação expirado."));
        }
        ...
        // Usa o token REAL do Identity para confirmar
        var result = await _userManager.ConfirmEmailAsync(user, storedToken.IdentityToken);
        
        if (!result.Succeeded)
        {
            return Result.Failure(new Error("Identity.ConfirmEmailFailed", "Falha ao confirmar email. O token original pode ter expirado."));
        }
    ...
    public async Task<Result<string>> GenerateEmailConfirmationTokenAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        return await _userManager.GenerateUserTokenAsync(user, "EmailConfirmation", "EmailConfirmation");
    }
